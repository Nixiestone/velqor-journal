// VELQOR JOURNAL — Database (Firestore)
import {
...
export const DB = {
  ...
  async addSetup(uid, setup) {
    return await addDoc(collection(_db, 'users', uid, 'playbook'), {
      ...setup, createdAt: serverTimestamp(), updatedAt: serverTimestamp()
    });
  },
  ...
  async addReview(uid, review) {
    return await addDoc(collection(_db, 'users', uid, 'reviews'), {
      ...review, createdAt: serverTimestamp(), updatedAt: serverTimestamp()
    });
  },
  ...
  async addMilestone(uid, ms) {
    return await addDoc(collection(_db, 'users', uid, 'milestones'), {
      ...ms, createdAt: serverTimestamp()
    });
  },
